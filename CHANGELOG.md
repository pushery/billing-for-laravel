...
- **The recapitulative statement as a file, for an operator who does not report it from DATEV.** `billing:recapitulative-statement:export` writes one row per VAT id and kind of supply for a quarter's reverse-charged sales to businesses in other member states, defaulting to the quarter that has just ended, or for a month with `--month`. A correction counts in the period it is issued in, with its sign; a reverse-charged sale that names no VAT id stops the export with `RecapitulativeStatementIncomplete`. Every run is recorded in `billing_tax_return_exports`, whose new `return_type` column tells it apart from the one-stop-shop return, and a month is recorded under the quarter it falls in. A jurisdiction profile states the deadline through `SuppliesRecapitulativeStatementDeadline`, and a limit on a quarter's supplies of goods with the monthly deadline through `SuppliesMonthlyRecapitulativeStatement`. The German one gives the 25th day after the quarter (§ 18a Abs. 2 UStG), sets the limit at 50,000 euros and the monthly deadline at the 25th day after the month (§ 18a Abs. 1 UStG); where goods pass the limit in a quarter or in one of the four before it, the quarterly export refuses and names that quarter.

- **A seller of goods who trades regularly is asked whether they sell as a business, before they sell more.** A sale of goods under intermediation now records its archetype, and `SellerActivityCounter` counts a seller's goods sales and proceeds over the calendar year, naming the sale that reached the threshold (30 sales or EUR 2,000.00, either one, configurable under `billing.marketplace.seller_activity` with its comparison and window). After it, the next sale of goods is refused with `TradingStandingUndeclared` until the seller declares their standing. A private individual's sale of goods is also refused with `SellerRecordIncomplete` while their record lacks what the profile requires, name, address and date of birth under the German one. `SellerStandingDisclosure` tells a checkout whether the seller sells as a business, whether the buyer has consumer rights and whether an invoice is issued, all from one answer. A migration adds `tax_archetype` to `billing_merchant_charges`.

- **A card can be taken at the counter, on a Stripe Terminal reader.** The new `CardPresentPayments` contract pairs a reader with a location, reads whether it can take a sale, puts a sale on it and takes a sale off it. The Stripe driver binds it; the Mollie driver binds nothing, so ask the container first. A sale is taxed in the country of the reader's location before the reader is asked, and a reader that is offline, busy or standing at no location with a country is refused with `ReaderUnavailable`. When Stripe confirms the card, the sale gets its receipt with the tax decided at the counter: a simplified invoice up to `billing.card_present.small_amount_threshold_minor` (€250 by default, the German small-amount limit), a payment record above it, numbered in your invoice series. The path takes cards only, which is what keeps the package from being a cash register under § 146a AO. Sales are kept in the new `billing_in_person_sales` table, so run the migrations. The documentation page names where the path stops: no offline sales, and in Austria each sale also belongs in your own registered cash register.

- **A buyer at the counter can ask for a full invoice.** `InPersonReceiptIssuer::reissueAsFullInvoice()` restates a receipt from the counter as a full invoice with the buyer's name, address and VAT id. It gets its own number, names the receipt it restates so the sale is counted once, and takes its tax from the receipt, because a VAT id does not move goods handed over at the counter. The receipt the buyer already holds is left as it was.

- **A seller whose record stays incomplete is reminded twice and then held, as stored states.** `billing:seller-data:escalate`, scheduled daily with the marketplace on, sends a first reminder after 7 days and a second after 30, each naming the missing details as `SellerDataReminderNotification` in all shipped languages, and applies a measure after 60: `withhold_payout` or `suspend_sales`, from `billing.marketplace.seller_data_escalation.measure`. A measure follows only where a missing field is required of that seller now, so a seller the reporting duty does not cover is reminded and never held. Each step keeps its distance from the one before, so a reminder that went out late moves the measure back. A withheld share stays pending on its sale and is paid out, with everything that waited, once the record is complete; no withholding outlasts `payout_deadline_days`, and `on_withholding_exhausted` decides whether a withholding that ran its course becomes a suspension. Every episode is kept on `billing_seller_data_escalations` with the channels each reminder reached. Bind `SuppliesSellerRecords` and name your record screen in `billing.marketplace.seller_record_route`.

- **A creator is reminded before their tax attestation runs out, not only once it has.** `billing:tax-holds:remind`, scheduled daily, tells a creator when the renewal falls due at the year boundary and again `billing.tax_small_business.reattestation.remind_days` (14 by default) before the attestation runs out. Each reminder names the day selling and payouts pause if nothing is renewed, goes out once per attestation, and is delivered as `ReattestationDueNotification` in all shipped languages. Name the route of your own renewal screen in `billing.marketplace.tax_standing_route` and the reminder links to it. `CreatorReattestationDue` carries it for a host that routes its notices its own way. A migration adds the two reminder markers to `billing_creator_tax_statuses`.

- **A creator standing corrected into the past issues the settlements since then again.** Recording a standing with a start date in the past queues every settlement of that creator from that date on, and `billing:settlements:restate`, scheduled daily, cancels each one and issues it again under the standing in force at its supply date, dated the day it runs. A settlement that already says what the corrected standing requires is left alone. When a replacement cannot be issued, the settlement stays as it is and the reason is recorded, and receipts on the buyer's side are never touched. The package moves no money here: listen for `SettlementRestated` and settle the difference in your payout process. Two migrations add `billing_settlement_restatements` and `billing_invoices.restates_invoice_id`.

- **`BuyerPartyResolver` names the customer an invoice of the local engine is addressed to.** Bind it to return the customer's name and address as a `Party`; it is asked once, when the invoice is raised, and its answer is frozen onto the document. The default names nobody.

### Fixed

- **The plausibility check no longer tells a commission-chain seller that their quarters do not add up to their year.** Once a chain stopped reporting the platform's margin as a withheld fee, `QuarterCoverageRule` went on comparing the quarters with what the platform kept. Every chain seller whose sales carried a margin got a finding, and `billing:reporting:run` refused the period until somebody acknowledged it. The rule now reads the year from the same records the quarters come from, including sales arranged as an intermediary. `UndocumentedSettlementRule` no longer names such a sale as lacking a settlement document, because intermediation never issues one.

- **A collective settlement books its creator input once per rate.** One document settles a creator's month, and a month can hold supplies at 19% and at 7%. The DATEV export booked the whole document to one input account chosen by a rate the document does not carry, so every collective settlement booked as a 19% input: a 7% month was not refused where no reduced account is confirmed, and a reverse-charged 7% month missed the reduced §13b account. Each line of a collective settlement now records the rate of its supply and its tax, and the export books one row per rate, adding up to the document. Collective settlements issued before this version book as the single row they always did.

- **A credit note against a reverse-charged sale is booked the way the sale was.** A local credit note and the correction a redeemed proration credit raises carried only the rate and the one-stop-shop fields of the invoice they reduce. Against a sale to a business in another member state, which accounts for the tax itself, the note therefore read as a domestic sale: the booking export put it on the domestic revenue account without the buyer's VAT id, and its e-invoice did not state the reverse charge. Both now carry the whole tax position of the invoice, listed once as `InvoiceRecord::TAX_POSITION`.

- **A creator whose tax standing is unestablished is not paid out while the hold is in force.** `blocks_payouts` was never asked by anything that moves money, so a share released from buyer protection or retried after a failed transfer went out regardless. Every path that moves a merchant's share now asks one payout gate, which holds the share on its sale until the standing is recorded or the money reaches `payout_deadline_days`.

- **A buyer-protection hold released after its sale's share already moved no longer transfers it again.** When the retry of a failed transfer had moved the share, a later release of the hold instructed a second transfer under the same key, which the provider only collapses inside its replay window. The release now sees that the sale has settled and completes without a transfer.

- **A marketplace receipt for a reverse-charged sale is marked as one.** `FanReceiptIssuer` wrote the reverse charge only as the receipt's exemption reason and left `reverse_charge` unset. The DATEV export routes a reverse-charged sale by that flag, so a sale to a business in another member state booked to the standard revenue account without the buyer's VAT id in field 40, and the distance-selling threshold counted it as a sale to a consumer. The flag now follows the reason on the receipt.

- **A routed sale books its revenue once in a DATEV batch.** Under the commission chain a sale has two documents, the buyer's receipt and the creator's settlement, and a period batch carries both. Each of them booked the sale to the buyer, so the revenue account received it twice and its automatic function derived the output tax twice. The receipt now books the sale, against money in transit, with its own rate, destination and relief; the settlement books the creator's input and the payout. A correction of the settlement reverses those two, and the correction of the receipt reverses the sale.

- **A settlement the self-billing engine issues now records its rate.** `SelfBillingEngine::issue()` left `tax_rate_bps` empty on every settlement, whatever the document stated. The DATEV export decides by that column, so a 7% creator input booked to the 19% input-VAT account instead of being refused until a reduced account is configured, a 7% reverse charge booked to the standard §13b account, and a correction of a settlement stated its tax beside a rate of 0%. The column now holds the rate the document states, and a new column, `supply_rate_bps`, holds the rate the supply is taxable at, which is what a reverse charge is booked by. The reverse-charge transaction key of a settlement is qualified with that rate too; without it, no settlement the engine wrote carried the key. The upgrading guide covers the migration and the settlements issued before it.

- **A seller whose sales the platform arranged as an intermediary appears in the reporting period.** Intermediation produces no settlement document, and `SellerReportingPeriod` read its roster and its figures from settlement documents alone, so a seller who only sold that way was in no report. The period and `SellerReportingRun` now also read those sales from `billing_merchant_charges`: what reached the seller (the price less the platform's fee, less anything taken back from them), how many sales there were, and per quarter the fee the platform charged the seller, net of what it refunded. A sale recorded before its kind was comes back as an unclassified line, and one recorded before its posture was counts under the posture the installation sells under.

- **A tax-free sale to a business across a border now books to its own DATEV account.** With a chart selected, a reverse-charged sale to a business in another member state was booked to the standard revenue account (SKR03 `8400`), an automatic account that derives domestic tax from the posting, and no booking named the recipient's VAT id. It now books to `fan_revenue_eu_reverse_charge` (`8336`, SKR04 `4336`) with the recipient's VAT id in field 40, which the recapitulative statement is read from, and a sale to a business outside the union books to `fan_revenue_third_country` (`8338`, `4338`). A published chart without the two accounts refuses such a sale instead of booking it as domestic; the upgrading guide shows the lines to add. Without a selected chart the export is unchanged.

- **A goods seller paid exactly EUR 2,000.00 is now reportable under the German reporting profile.** The small-scale exemption was read as "did not exceed", the wording of the directive, so a seller with fewer than 30 sales and exactly EUR 2,000.00 was left out. § 4 Abs. 5 PStTG exempts only a seller paid less than the figure, and a German platform files under that statute, so `billing.reporting.goods_de_minimis.compensation_operator` now defaults to `<`. The marketplace guide and the configuration reference say so, and the upgrading guide shows how to keep the previous comparison where a statute follows the directive.

- **A local invoice to a business now names its buyer, by the VAT id its tax was decided on.** The local engine raised every invoice without a buyer, so the document addressed nobody, the e-invoice gave Germany as the buyer's country, and a reverse-charged invoice to a business in another member state left out the buyer's VAT id the zero rate rests on. The invoice now carries the VAT id a register confirmed and the country it registers the buyer in, and the name and address your `BuyerPartyResolver` returns. Where the two disagree on the id or the country, the tax facts win, because the document has to state the id the supply was decided on. A consumer's invoice under the default binding is unchanged.

- **`billing:reporting:run` resolves on an installation that bound no reporting rule of its own.** `ClassifiesReportability` had no binding, so the reporting run could not be built unless the application bound one. It is bound to `GermanReportingProfile` by default, like `ReportingProfile`.

- **The finding about a chain sale no settlement document names now says what the report does with it.** It said such a sale was placed in the quarter of its own settlement date and that its fee was still reported. Neither was true: the reporting period counts a chain seller's figures from settlement documents alone, and a chain charges the seller no fee. The sale is in the figures only if a collective settlement covers it without naming it, as one does when its run named no charges. `settlement_document_does_not_place_this_charge` now says so and asks to confirm that a document covers each sale, or to raise the missing one and re-run. The figures do not change.
...
- **Four packages the shipped code imports directly are now declared in `require`: `nesbot/carbon`, `guzzlehttp/psr7`, `psr/log` and `symfony/http-foundation`.** All four were reached only through somebody else's dependency tree — `Carbon\Carbon` and three siblings, `GuzzleHttp\Psr7\HttpFactory`, `Psr\Log\LoggerInterface`, and `Symfony\Component\HttpFoundation\Response` with `StreamedResponse`. **Nothing new is installed**: the lockfile holds 194 packages before and after. What changes is who promises their version — an undeclared import is one Composer may resolve below what this code calls, and the failure then surfaces as a missing method in a class this manifest never names. Three of the four are floored at the base of their major, which is what this package's floor pin insists on. ⚠️ **`guzzlehttp/psr7` is floored at `^2.0` rather than `^1.0`, and says why in the pin**: `HttpFactory` is the PSR-17 factory that arrived *with* 2.0, so a base-of-the-major floor there would promise a version the import cannot run on.

- **A team can be asked whether the acting user still belongs to it, before billing follows it.** In team mode the owner is whatever `billing.team_relation` returns off the acting user, and for a relation that *is* the membership that answer carries its own proof -- the row exists because the user belongs to the team. A great many applications instead point at a mutable column the user controls, `current_team_id` being the usual one, and there the two questions come apart: the pointer keeps answering after the user has been removed, so billing would keep following a team they left. The package cannot tell the two apart from outside, and guessing at method names on someone's team model would be the same mistake in a different hat -- so the owner declares it. A team model implementing the new `Pushery\Billing\Contracts\VerifiesBillingMembership` is asked, and a `false` answer falls back to the acting user, the same direction the resolver already takes when team mode yields no team at all. **Nothing changes for a consumer that does not implement it**: an owner that cannot be asked is trusted, exactly as today, and the arm pinning that is part of this change rather than left implicit. `config/billing.php` now says which shape of relation needs the seam and which does not.


- **An Adyen webhook verifier, and the endpoint now has something to authenticate with.** Adyen differs from both drivers already here in a way that decides the shape: Stripe signs the request and Mollie signs it or sends a bare id, while Adyen signs each `NotificationRequestItem` *separately, inside the body*, and posts several of them in one request. There is no request-level signature to check, so a request is authentic only when **every** item in it is — one forged item among nine genuine ones fails the whole request rather than being dropped quietly, because the effects run per item and nothing downstream would know which ones were vouched for. The signature covers the item with `additionalData` removed, so amount, merchant reference, event code and success flag are all inside it, and nothing in the verifier reads a field before the check passes: deciding whether to verify by looking at `eventCode` would make that decision out of unverified input. Optional HTTP basic auth on the notification endpoint is checked too, and **skipped rather than failed** when no credentials are configured, since the HMAC alone is Adyen's documented default and requiring credentials unasked would refuse every install that follows it. `adyen/php-api-library` is a suggestion rather than a requirement — the same reasoning that keeps the Mollie client optional — and without it the verifier refuses everything: an endpoint that cannot check a signature must not be the one deciding a payment succeeded. **Nothing is wired up yet**; binding this into a driver is separate work, so no existing install changes behavior. The SDK helper is named through an optional constructor argument rather than a hardcoded constant, and the presence check is an `is_a` rather than a `class_exists`: the one call answers both questions — the class is installed, and it is the helper whose method this code then calls. A name that resolved to something else would pass an existence check and fail at the call site with a fatal, which is a worse failure than the refusal the branch exists for. The argument is there because the absence branch is the one an install without the optional package walks, and a suite that has the package installed cannot otherwise enter it — a guard nothing can reach is a guard nobody has checked.

- **A fifth plausibility rule: a charge that no settlement document places.** The consideration credited to a seller and the fee withheld out of it are both placed by the settlement document that credits them. A charge no document claims has no such date, so it keeps its own settlement date — correct, and not nothing: one line of the quarter was placed by a different event from the rest, and somebody reconciling that quarter against the seller's own documents will not find it there. The run now names those rows per quarter, with a sample of their references, instead of leaving them to be discovered. The fee is still reported either way, because a fee nobody reports is as wrong as one reported twice and is the quieter of the two. Like the four rules it joins, it is structural — it names no country, no threshold and no form.

  **Upgrade — this rule can refuse a period that used to pass.** `ReportingPlausibilityGate::assertClear()` throws while any finding is open, so an installation holding charges whose settlement documents were never raised, or whose historical documents name no provider and therefore cannot be matched, will see `settlement_document_does_not_place_this_charge` and must answer it. Two answers are available and both are real: raise the missing document and re-run, or acknowledge the finding for that period once the placement is accepted. An acknowledgement clears one finding for one period, so a charge still undocumented next quarter is reported again — that is new information about the same gap rather than a repeat. To see the rows without running the gate, ask `WithheldFeeCounter::chargesPlacedByTheirMoneyIn()`, which is the same call the rule makes.

- **A collectively settled transaction can now state what was sold, on its own line.** A collective settlement puts a creator's whole month into one Ultimo-dated document whose transactions are lines, and its header deliberately carries no tax characteristics: a creator who sold a download and a commissioned piece in the same month has no single archetype, and a column that is sometimes right is worse than an empty one. A **line** has exactly one. `SettlementTransaction` now takes an optional `characteristics`, and the line it becomes freezes the archetype, what it was sold alongside, the place-of-supply rule and the rate category — beside the `tax_rate` and `service_date` it already carried — and, where the transaction named one, **which routed charge the line settles**. The charge-side link reaches the document, which is the whole answer for a per-transaction settlement because there the document *is* the transaction; a collective document carries a month, so a correction of one transaction has to find its line, and matching on the date and the amount would be a guess that two identically priced sales on one day get wrong. That the per-transaction engine keeps these on its *header* is not a different rule: there the header **is** the line, so the distinction cannot show. Collective settlement is the case that reveals them as line-level facts.

  **Upgrade:** nothing to do. The parameter is optional and last in the list, so no existing call changes, positionally or by name. A line whose caller stated no characteristics keeps them null, and that absence is written rather than guessed — it is not the same as the header's emptiness, and the difference is what lets a correction of that line refuse instead of issuing a document with invented characteristics.

### Changed

- **`stripe/stripe-php` 21 is adopted, and the pinned Stripe API generation moves to `2026-08-26.dahlia` with it.** Cashier v16.8.0 widened to `^21.0` on 2026-09-01; this package stopped at `^20.0`, which cut off a major Cashier itself vouches for. The lower bound is untouched at `^17.4` — this widens the range, it does not raise a floor. What made the adoption more than a version bump is that Cashier sends `StripeApiVersion::CURRENT`, a constant that moves with the SDK major, so adopting 21 means both clients address one Stripe account in one generation or in two. Two generations is the state where the same object comes back in two shapes and both calls succeed, which is why the package pins its own version and holds it against Cashier's. ⚠️ **What is proven and what is not, stated rather than left to the diff:** 21.0.0 carries a single breaking change, a PHPDoc retyping of `Stripe\ErrorObject` this package reads nowhere; `Invoice::$id` is byte-identically non-nullable across 20.3.1 and 21.3.2; the call sites and imports are untouched; static analysis is clean at max with 21.3.2 installed and a cold cache; the faked suite is green. **None of that is evidence about the API generation** — a changed response shape is precisely what a faked suite cannot see. The generation was moved on an explicit instruction rather than on a live run, and the live smoke against a real key remains worth running the first time one is at hand.

- **The tax-point switch says what it is: Soll- or Ist-Versteuerung, and an election rather than a country's rule.** `billing.tax_point_on_receipt` has decided since it was built whether the tax on a period falls due when the service was rendered or when the money arrived — but it was the one key in its neighborhood carrying no comment at all, and the German words for the two, which are the words on the notice an operator holds, appeared nowhere in the package. An operator with Ist-Versteuerung had no way to find the switch that implements it. The key now carries who may apply (§ 20 Abs. 1 UStG, all four alternatives), which legal forms usually land where, and why the default being off is the statute rather than a preference: accrual is the normal case under § 16 Abs. 1 S. 1 UStG, and receipt taxation exists only on application and approval. **The correction that matters most is a single word in the class docblock**, which described the choice as *"a jurisdiction's rule, read from its profile"*. It is not. Two operators in the same country differ, because one of them applied and was granted it — so this can never be derived from a locale, a currency or a tax profile, and a profile that set it for a whole market would put every transaction of everybody on the other basis into the wrong period. It is declared once, by the operator who holds the notice. No behavior changes.

- **The marketplace guide now states the reporting rule the package ships, the statute behind each branch, and the fact that the whole thing is an interpretation.** A consumer installed the package and received a wired DAC7 classification — standardized supplies left out, individually commissioned ones reported, consumer goods reported unless both de-minimis edges hold — with nothing on the page saying which norm each branch reads or that the line between a standardized product and a commissioned one is a judgment about their own catalog. Four things are written out now. The two edges of the goods exemption are cumulative and are named with the figures actually wired, including the worked boundary: a seller on exactly EUR 2,000.00 with fewer than 30 sales is exempt, because the statute exempts whoever "did not exceed" the figure, so the shipped comparison is `<=` and a strict one would file somebody the law leaves out. There is no de-minimis outside the goods branch. Acting as a commissionaire does not exempt anybody — the VAT rules that make you the seller decide the VAT treatment and say nothing about this duty, which is the wrong-statute conclusion most often reached for. And both directions of error are punished, so filing in doubt is the second mistake rather than the cautious one. A new arm derives the figures from `config/billing.php` and fails while the prose still carries the old ones; the numbers live in the configuration, not in a third copy.

- **BREAKING (pre-1.0) — A DATEV batch containing a foreign-currency document that never froze a rate is now refused instead of exported.** Such a row went out with fields 4–6 empty, and one docblock called that correct while another in the same file described what it does: DATEV's rule is that a row whose currency differs from the batch's base must carry the rate or the base amount, and a row carrying neither is either rejected by the import or **booked at face value** — 500,00 PLN posted as 500,00 EUR, overstating the revenue by the exchange rate with every figure looking plausible the whole way through. The export now refuses the batch, which is the answer it already gives to four other unexportable states. **Refusing is not dropping:** the objection that dropping the row would hide revenue is right, and it is an objection to dropping — a refused batch exports nothing and says why, so nothing is hidden, nothing is posted wrong, and the missing freeze becomes the next action rather than a reconciliation months later. What settled it against sending the row is that sending rests on the import *questioning* it, and the same file says the import may instead book it silently. Deriving a rate at export time stays off the table: that is the divergence freezing exists to prevent.

  **Upgrade — an export that used to succeed can now throw.** `InvalidDatevBatch` carries the document number, its currency and the batch's base currency, and names the two ways out: freeze the document-layer rate on that document, or export a period that does not contain it. A single-currency installation is unaffected — a row already in the base currency takes the branch that legitimately needs no rate, and that is now the only case in which none is expected. `documentRate()` returning null therefore means exactly one thing where it used to mean two.

- **BREAKING (pre-1.0) — The withheld-fee figure a DAC7 return states is now placed by the settlement document, in the same quarter as the consideration it was deducted from.** A return states the value credited to a seller and the fee the platform withheld for the same quarter, and the two were placed by different events: the value by the settlement document (`issued_at`), the fee by the charge's own settlement date. On the ordinary sale they coincide. When they do not — the money moves on 31 March and the document is issued on 1 April — the old reading put the fee in Q1 and the consideration it came out of in Q2, so the Q1 pair stated a deduction from something that quarter did not contain. Neither figure was implausible on its own, which is why no plausibility rule could catch it. A withheld fee is not an event of its own; it is a deduction *from* a particular consideration, so its quarter is that consideration's. **A charge no settlement document claims keeps the money clock** — the money can move before any document is raised, and a legacy document naming no provider cannot be matched to its charge — and such a charge is never dropped, because a fee nobody reports is as wrong as one reported twice and is the quieter of the two. The new `chargesPlacedByTheirMoneyIn()` names those rows so a plausibility step can state the fallback rather than a reader discovering it while reconciling.

  **Upgrade — two changes to make.** The figure moved to a counter of its own: `MerchantChargeAnnualEarningsCounter::feesWithheldIn()` is gone and the same call now lives on `WithheldFeeCounter`, constructed exactly like its siblings (`new WithheldFeeCounter($config)` or resolved from the container). That counter keeps its own clock for its own duty — what a seller actually received, for a small-business threshold — and a counter with two clocks was the defect, not a convenience. Reported figures for past quarters **will move** where a document and its money fell on different sides of a quarter boundary; a filing already submitted is unaffected, but a re-run over an old period can produce different quarterly splits whose annual total is unchanged.

- **The troubleshooting page explains the batch refusal for an unfrozen foreign-currency document, and the new settlement-line refusal.** `InvalidDatevBatch` listed three refusals and had gained a fourth; the page now names all four and says what to do about the new one — freeze the document-layer rate, or export a period that does not contain that document — plus the reason deriving a rate at export time is never the answer. And `SettlementLineNotFound` has its own entry beside the other collective-settlement refusals, explaining why a correction is refused rather than computed from a header that states no commission, no rate and no fan gross for a month, and why the check asks whether the line can *answer* rather than whether one was found.

### Fixed

- **A reversal driver bound under `ReversesMerchantShare` is actually reached now.** The contract's own docblock explains at length why the reversal is a **sibling** interface rather than a second method — *„a consumer registers its own driver through the public extension point"* — which reads as an invitation to implement it in a class of one's own and bind it there. That binding was never read: the chargeback job resolved the **outbound** verb, `MovesMerchantShare`, and settled the question with an `instanceof`. Measured across the package: one consumer of the contract, **zero** `make()` calls for it, **zero** bindings. ⚠️ **Nothing looked wrong from the outside** — the refund attempt was recorded as failed with a message saying the driver could not reverse, when the truth was that the consumer's reverser was never asked, and a consumer reported the cost before anyone noticed: their outbound driver is a wallet of their own, so every lost dispute left the creator holding the share of a sale the network had already clawed back. No throw, no log line, nothing that looks like a defect. The package's own driver hides it, because `StripeMerchantTransfers` carries both verbs on one class and satisfies the unwritten condition by accident. **Nothing changes for an installation that binds only the outbound driver:** the job asks for the reversal contract first and falls back to the outbound object exactly as before, which is the shipped shape.

- **The failure message tells a missing binding apart from a driver that cannot reverse.** One sentence covered both, and they need different actions: *nothing is bound* is a destination-charge installation and usually correct, while *bound, and it cannot reverse* is a capability gap somebody has to close. A sentence covering both is the sentence nobody acts on.



- **A refund on a collectively settled sale now issues its correcting document.** It issued none at all: the lookup that finds the document to correct searched the document side, and a collective settlement carries a month and names no charge there — so a refund inside a settled month corrected nothing, and the collective invoice kept stating revenue the creator no longer had. Settlement numbers come from a gapless series, so that cannot be tidied afterwards, only answered by a further correctly numbered document. The charge now records which settlement paid it out on both settlement paths, so the lookup asks the charge first: a recorded fact rather than a match, and the only direction a collective settlement has. **The correction reads its figures off the LINE**, because a month has one of no archetype, no commission and no fan gross, while a line has exactly one of each — `lineSettling()` resolves it in one place, since the arithmetic and the document need the same line.

  **Upstream of that, a collective settlement line now carries what it is about:** the archetype and what it was sold alongside, the place-of-supply rule and the rate category, the commission terms and tax rate the transaction was priced under, what the buyer paid for it, and which routed charge it settles.

  **Upgrade — one new refusal, and it is deliberately total.** Where a settlement covers a period and no line of it *answers* for the charge — none names it, or the one that does was written by an older version without the commission rate, the tax rate or the fan gross — the refund raises `SettlementLineNotFound` naming the document and the charge, rather than computing the correction from a header that states none of them. Each missing figure fails differently and all of them look equally complete: an absent commission books the reversal as if the platform kept nothing, an absent fan gross books it as 0.00 against a real sale. Re-run the collective settlement for that month so its lines carry the figures. **A per-transaction settlement is unaffected** — its single line names no charge because there the header *is* the line, and its readers take the header as they always have.

- **A per-transaction settlement now records on the charge which document settled it, the same way a collective one does.** The link between a routed charge and the settlement that paid it out existed in two different directions depending on how the creator was settled: a per-transaction settlement named the charge on the *document* (`settled_charge_reference`), while a collective one — whose transactions are lines and which therefore cannot name anything in its header — recorded the link on the *charge* (`settlement_invoice_id`). One fact reachable two ways is how a reader ends up covering half the installations: walking from a charge found only collective settlements, walking from a document only single ones, and neither could tell which half it was missing. Both paths write the charge-side column now, so the direction is one. The document keeps its own reference as well — every correction finds its original through it, and dropping that to tidy up would break a different promise. **Upgrade:** nothing to do, and nothing changes for a reader of the document side. An installation that settles per transaction gains a column it was leaving null; existing rows are filled by a migration that matches on the merchant, the provider and the reference together, so a settlement never claims a charge it does not name.

## [0.32.0] - 2026-09-17

### Added

- **A paid credit balance reaches the books, on both sides.** A top-up landed on the owner's balance and appeared in no DATEV booking at all: the trace existed as an audit row and as a ledger entry, and neither is a bookkeeping movement. The balance is a multi-purpose voucher in law — money held against a promise, taxed when it is finally spent — so a top-up books as a **liability** against money in transit, on the same account the voucher product uses, and a refund reverses both sides. **The redemption settles the receivable, not revenue:** the invoice the balance paid towards has already booked its own revenue, on the local lane and on the provider's alike, so a second revenue row would state the same turnover twice. A month with a top-up and a full redemption leaves the liability account at zero, which is the arm that makes "both sides or neither" checkable rather than a claim.

  **One movement is deliberately not booked, and the batch says so.** A proration credit grants spendable balance with no payment and no credit note behind it, so what it is in the books is an open question rather than a lookup. The period batch reports its amount as `unbookedCreditMinor` instead of inventing a liability — a zero there and a zero because nothing happened are different facts, and only the figure tells them apart.

  **Upgrade:** nothing to do. An installation that sells no credit add-on produces the same batch as before; one that does gains rows it was missing, on the liability account its chart already names. A caller that reads the period batch's array gets two new keys and no changed ones.

- **A redeemed proration credit now raises a correcting document against the invoice it came out of.** A proration credit hands back part of the consideration for a period nobody used, but nothing is refunded — the money stays with you as spendable balance. The reduction becomes real when that balance is finally spent, and at that moment the consideration for the *original* supply has been reduced: § 17 Abs. 2 Nr. 2 UStG corrects the old invoice, not the new one the balance was set against. The document is an amendment (EN 16931 type 384, never 381), states a positive amount, and carries the frozen tax characteristics of the supply it reduces — so your booking batch resolves it to the same revenue account the original used, at the same rate. It is raised by the local billing cycle at the moment of the offset; where a provider raised the invoice, that provider announces its own corrections, so nothing is issued there. **A credit whose source invoice cannot be named is not corrected**, and that is deliberate: such a credit exists only where the reading behind it could not point at exactly one paid invoice, and correcting a document that never carried the supply — at a rate nobody chose — would be worse than reporting the gap. Credit notes against a locally raised invoice all draw from one number series, so the series an accountant reads stays gapless across both producers.

- **A routed charge now records what was sold, not only how much of it the merchant is owed.** The row carried three amounts and the lane they took, and nothing said what the buyer actually bought. That is invisible while the share is *transferred* — a transfer needs an amount and a destination, and no more — and it becomes load-bearing the moment an application credits the share to a balance instead, which is exactly what the `MovesMerchantShare` seam exists to let you do: a ledger entry carries a type, and an append-only ledger with the wrong one cannot be corrected, only offset. So the one seam offered for a balance-based payout could not be served without the consumer keeping its own parallel table of what each charge was for. `MerchantCharge::$purpose` answers it directly, as `MerchantChargePurpose::Subscription`, `AddonPurchase` or `Tip`.

  ⚠️ **It is a column rather than an argument on the seam, and the reason is a fatal error rather than a preference.** `MovesMerchantShare` is implemented **outside** this package — that is the whole reason it is a separate interface instead of a method on `PaymentRails`. In PHP an implementation that declares fewer parameters than its interface fails at **class load**, so adding even one trailing optional parameter would make every existing consumer driver fatal on `composer update`, with no graceful path and no runtime warning first. Measured against a control rather than assumed. The row is the one place a driver can read this from without the package reaching into code it does not own, and `charge_reference` is unique per provider, so the lookup from the source charge it is already handed is exact.

  **It is derivable afterwards for two of the three lanes and not for the third, which is what makes it worth storing.** A one-off purchase resolves through its payment reference to an addon key. A subscription cycle takes three hops — the reference is an invoice, which names an order, which names a subscription — and the middle hop only exists where this package writes the order. A tip persists nothing that names it at all: it is reachable only by the **absence** of the other two, and absence during a race reads a purchase whose rows have not landed yet as a tip. The lane knows it for certain at the moment it opens the checkout; every route back to it afterwards is longer, and the last one is wrong.

  **Null means "written before this was recorded"**, the same meaning `charge_type` and `seller_posture` already give it on this table, and old rows are deliberately **not** backfilled — the guess would be unreadable forever for exactly the tip case above. Nothing changes for an installation that transfers rather than credits: the column is additive, the seam is untouched, and a driver that does not read it behaves as it did.

- **`SubscriptionStateChanged` carries the local code of a redeemed merchant coupon, so a hosted checkout can book the redemption.** A minted merchant coupon is enforced locally on purpose — the provisioner does not send `max_redemptions` to the provider, because two counters drift apart the moment somebody edits the row. That makes your `billing_coupons` row the one authority, and on a hosted lane it was an authority nobody ever told: the discount is applied inside the provider's checkout session, your application is not in that transaction, and nothing afterwards said which code took effect. A creator who set "100 redemptions" had nobody holding it — `redeemed_count` stayed at 0 and the provider had never been given the limit either. The new `$couponCode` closes that: read it off the event and run your redemption through `CouponRedeemer` as you would on any other lane. It is the code from your own row, not the provider's id, so booking it needs none of the provider arithmetic this package exists to hide. **`null` is the common answer and means one of two things, both fine:** no discount was applied, or the discount came from your `billing.coupons` catalog — where you created the provider coupon yourself, with its own limits, and the provider is already counting. Nothing changes for a catalog coupon, and nothing changes for a non-hosted lane, where the redemption already went through `CouponRedeemer`.

### Changed

- **A chart of accounts with no entry for a transaction is a sentence on the admin console, not an error page.** The export refuses rather than booking to a default account, which is right — a wrong posting imports cleanly and surfaces only when a tax advisor reads it. What was wrong is where that refusal landed: the scheduled command answered with a line naming the transaction and the configuration path, and the same misconfiguration on the screen beside it was an unhandled exception. It now takes the same route as every other refusal there, carrying the package's own words, which name the transaction and where to configure it — more than a heading of ours could say. ⚠️ **The two refusal types are named rather than caught by their shared ancestor.** Both extend `RuntimeException`, and catching that would have been shorter and would have swallowed every genuine defect on the one screen that hands out a complete revenue history; an arm holds an unrelated failure to still coming out.

- **The admin console states it too, so the figure does not depend on which door the file left by.** The command reported the unbookable credit and the screen beside it did not — an operator who takes the batch from the browser instead of from a shell got a complete file with an open question in it and nothing said about it. It now reports the same sentence, with the same zero, in its own polite live region rather than in the assertive one beside it: an unbookable grant is not a failure direction, the file is correct and complete about everything it does book, and ranking it with a refusal would say something false about both. The figure belongs to the period that was exported, so it is cleared the moment a bound moves or an attempt produces no file — a statement about a batch nobody downloaded is worse than none. ⚠️ **It was deferred once on a belief that turned out to be measurable and wrong:** the screen warns against depending on state set before a raw stream, which is true of a raw response and not of this one — Livewire captures a returned download into an effect and completes the request normally, so the same response carries the file and the figure. The arm asserts both together rather than restating the belief.

- **`billing:datev:export` states the credit movement it could not book, and counts credit movements in its summary line.** The batch has computed that figure since credit movements reached the export and argued for it in its own comment — *"puts the gap in front of the person who runs it"* — while nothing read it: across the whole package the key appeared in its own definition and in test assertions, and in no surface at all. The person running the batch never saw the number. A proration credit grants spendable balance with no payment and no credit note behind it, so the pair of accounts it would book against is an open question rather than a lookup; the file stays correct and complete about everything it *does* book, and this line is the gap. **It is printed even when it is zero**, which is the whole argument for it: a period that booked everything and a period whose credit movements were never loaded both produce silence otherwise. ⚠️ The figure is a **net** and can be negative — spending such a grant takes its share back out, so a period that granted and redeemed the same credit owes the books nothing and now says so. The summary line gains credit movements alongside invoices, provider fees and vouchers, which is the rule stated beside it and not followed: every count, always, including the zeroes. The exit code is unchanged and still belongs to the reconciliation.

- **`laravel/ai` is a `suggest` rather than a dev dependency.** It arrived in `require-dev` with `laravel/boost` and `laravel/mcp`, and unlike those two it is only ever called by an eval that scores through a real judge — this package's eval uses free expectations and asks no judge at all. It also does not travel alone: it requires `aws/aws-sdk-php` for a provider nothing here calls, which is 67 MB and 3512 files in every checkout and every CI lane. Nothing a consumer installs, calls or configures changes either way, and `anthropic-ai/sdk` has been a `suggest` for this same reason since the beginning; the two are now consistent.

- **The subscription docs now say when the first real charge lands.** The local engine bills in arrears — an order closes the period it is for — and the payment taken at checkout is a mandate verification of one minor unit by default, so a monthly plan collects nothing real for 30 days, 44 with a two-week trial, and a yearly plan for 365. None of that was written down: `arrears` appeared in the docs only in the dunning sense, and the subscriptions page never mentioned the first charge at all. Nothing about the behavior changes; the page now states it, names the lever that adjusts the wait without code (period length), and says why billing in advance is a different model rather than a setting.

- **A proration credit is now prorated from what the customer actually PAID, and it names the invoice it gives that money back from.** On a tier swap the unused remainder of the old plan becomes spendable credit. Until now that remainder was computed from the plan's **list price**, so a customer who had paid a discounted invoice was credited more than they ever paid — and the excess is a gift rather than a returned consideration, which is a different thing in your books and in the tax on them. The strategy now looks for the paid invoice of the period being prorated (same owner, same currency, `issued_at` inside the period) and prorates that invoice's total instead. **Amounts can therefore change for any customer whose invoice differed from the list price**; where they matched, nothing moves.

  The credit's ledger entry also carries that invoice as its `CreditSource`. A proration credit gives back part of an already invoiced and already taxed consideration, so the correction it eventually needs belongs to that invoice at that invoice's tax rate — and nothing downstream can reconstruct which invoice a balance movement came from, so it is written at the moment of the swap or not at all.

  **Where no single invoice answers, nothing is guessed.** No match and two matches are treated alike: the credit is written without a source and prorated from the list price, exactly as before, and the amount keeps appearing in the monthly reconciliation as a named difference. "The latest paid invoice" would be an ordering guess about which invoice a later tax correction belongs to, and a credit attached to the wrong invoice corrects the wrong taxable base. An application whose invoices come from a provider and fall outside the period is unaffected and keeps the honest unattributed number.

- **An invoice raised by the local engine leaves `subtotal_minor` null when no tax basis was established, instead of setting it equal to the total.** It already left `tax_minor` null there, for a reason that applies just as well one column over: a subtotal equal to the total is not a neutral placeholder, it is the statement that the supply was untaxed — correct under a small-business regime or an exemption, wrong under a taxable supply, and nobody had determined which. Nothing a reader sees moves: the rendered document derives the net from the total and the tax, and the e-invoice sums the frozen lines, which this issuer always writes. A document that carries a determined basis is unaffected and still freezes the net beside it.

- **`OneTimeCharge::tip()` says that a null answer has two causes.** It returns null both when tipping is switched off and when the buyer chose nothing, and only the first is a configuration answer somebody may want to act on. The information was always available — `tipsEnabled()` is public — but nothing pointed at it, so a caller had to infer a misconfiguration from a value that also means the tip box was left empty.

### Removed

- **The README no longer carries the `mutation-≥85%` badge.** It asserted a figure this package had never finished measuring, and a badge on the front page reads as an achievement rather than as a flag in a build recipe. Nothing about the package changes; one claim that could not be backed is gone from the page, and a guard now keeps it off until a finished run earns it.

### Fixed

- **A correction built as an AMENDMENT was stored without that fact and then rendered as a full cancellation.** `InvoiceCorrectionSnapshot` validates its `kind` — an amendment with nothing to reference is refused outright — and the persistence path wrote every other field and dropped that one. The e-invoice writer treats anything that is not an amendment as EN 16931 type code **381**, which says the original document was canceled in full; a partial correction therefore described itself that way to a tax authority, with every amount on it still adding up. **Nothing wrong has shipped:** the one correction source today states no kind, so the default and the rendered 381 agreed. What was missing is the road for the other case — anything of yours, or a second driver, building an amendment got no error, just a silently mistyped document. It now keeps the kind it was validated for, and a partial correction renders as **384**.

- **A credit note for a refund on an invoice this package raised itself booked every correction at the standard VAT rate.** The revenue account in a DATEV batch is resolved from the invoice's frozen tax characteristics, and those accounts are Automatikkonten — the export writes no tax key on purpose, so DATEV derives the rate *from the account*. The local credit note carried none of them, so a correction against a **7 % supply reduced 19 % of tax**, and one against an **OSS sale reduced domestic tax instead of the destination country's**. Nothing was ever red about it: the amount was right, the file well-formed, and the batch balanced against itself — the difference first appears in a VAT return. The note now copies the rate, the OSS flag and the destination country from the invoice it corrects, never re-deriving them, which is what the marketplace correction writers already did. **If you export DATEV batches containing local credit notes against reduced-rate or OSS sales, the affected periods need re-exporting** — the corrections in them point at the wrong account.

- **A sale whose tax the provider computed now leaves the buyer a document, and every figure on it is the provider's own.** Where `automatic_tax` is on, Stripe works the tax out from the address the buyer types into the checkout, and the completion event carries amounts and never a rate. This package therefore issued nothing at all on that lane — the honest refusal, because stating a rate nobody supplied puts a number on a tax document that no decision stands behind, and a stated tax is owed by whoever stated it whether or not it was ever due. The rate was not missing, though: it is on the sale's line items, one call away, beside the exact base and the exact tax. A new seam, `ReadsProviderComputedTax`, asks for it in the package's own vocabulary, and the Stripe driver behind it reads the line items. So nothing is derived and nothing is decided — the document states what was applied. The amounts are written as they arrived rather than recomputed from the rate, because the two can land a cent apart whenever the provider taxed the net and the rounding fell the other way, and a document that silently corrects the provider by a cent disagrees with the payment a reader holds it against. `FanReceiptIssuer::issue()` takes an optional `providerTax` for that; every existing call site is unchanged and still derives the split from the rate.
- **The rate is read from `effective_percentage`, never from `percentage`, and the difference is silent.** Stripe separates them: the statutory field "includes the statutory tax rate of non-taxable jurisdictions", while the effective one is "the rate actually used to calculate tax" and is the one defined for automatic tax. On an exempt, zero-rated or reverse-charge supply the two disagree — the statutory field still names the country's headline rate while nothing was charged. Reading it would put a rate on the document that was never applied, beside amounts that contradict it, from a source that looks authoritative.
- **A rate limit is not one of those refusals, and the exception hierarchy is why.** Stripe makes `RateLimitException` a subclass of `InvalidRequestException`, so the ordinary "could not read it" catch would also take the one failure that means *ask again in a second* — and file it as a permanent "no tax here". The caller would issue no document, the job would return normally and never be re-driven, and every provider-taxed sale inside a rate-limit window would quietly lose its receipt: exactly the defect this change exists to end, one layer further down. It is rethrown, so the job retries.
- **Four shapes are refused rather than interpreted, and a refusal still issues nothing.** Several rates on one sale, because a document states one rate against one base and averaging them would describe neither; a rate that is not a percentage, which Stripe reports for a flat-amount tax; a rate finer than the stored basis points, so that the rate on the document always reproduces the amounts beside it; and a provider that could not be reached, because a failed read is loud while a fallback computation is a plausible figure nobody questions. The caller checks one thing more: that the base and the tax add up to what the buyer actually paid. A reading that lost a line or took the wrong one still hands over a well-formed split, and only the sum catches it.

- **An invoice Stripe returns without an `id` is skipped from the invoice history instead of throwing a `TypeError`.** The driver had read the id straight into a `string` parameter on the belief that stripe-php 18.0 made `Invoice::$id` non-nullable. It did not: `id` is a docblock-only property on every major, and the SDK answers `null` for a key the payload does not carry — reproduced on v20.3.1, where the crash lands in the application rather than in the driver. Every other invoice on the page is unaffected and still renders; only the row that carries no identifier is left out, because there is nothing to open, download or reference it by.

- **A redeemed proration credit no longer debits the voucher liability for money that was never credited to it.** A proration credit grants spendable balance with no payment and no credit note behind it, so granting one books nothing — a booking there would create a liability nobody owes. Spending it, however, took the same route every redemption takes and released the liability for the whole amount. The account therefore ran into debit by exactly the proration share on every offset that touched one, and nothing about it looked wrong: the file stays valid and the batch reconciles against itself, so the only symptom was a liability account slowly turning into an asset. An offset now books only the part of it that has money behind it, split FIFO out of the credit ledger, and one paid for entirely out of a proration credit produces no booking row at all.
- **`unbookedCreditMinor` no longer describes a gap that has already closed.** The figure a period batch reports for credit it cannot book was the sum of the proration credits *granted* in the period. Spending such a credit now takes its share back out of that figure, so a period that granted and redeemed the same credit reports nothing outstanding — which is the truth about it, since neither half was ever booked.

- **A published config file no longer stops a later release's settings from arriving.** Laravel's `mergeConfigFrom()` is a flat `array_merge`, so it asks one question per top-level key: is it there? An installation that ran `vendor:publish` has every top-level key, so a setting this package added *inside* one of those blocks afterwards never reached it — the published block won whole, and the new setting simply read as `null`. Nothing errored and nothing was logged, which is why it could go unnoticed for releases at a time; 37 of the 69 top-level keys across `billing.php`, `account.php` and `license.php` are blocks with settings underneath them, including the payment drivers, the checkout, the dunning schedule and the tax evidence. All three files now merge underneath a published copy block by block. Recursion stops at any list, so a list you narrowed stays exactly as you wrote it and an emptied one stays empty — a merge that hands back entries you deliberately removed from an access or driver allowlist would be worse than the bug it fixes. An explicit `null` you wrote also stands, so switching something off that way keeps working.

  **Upgrade:** nothing to do, and for most installations nothing changes — if you never published the config, or published it and kept every block whole, you already had these values. If you published a trimmed block, settings you never saw start taking effect with this release. In one direction that deserves a look: a default that is deliberately strict was effectively absent for you and is now on. If you had relied on it being absent, write the value you want explicitly in your published file. An installation running `config:cache` is not merged at all — that is the framework's design, not a limit of this change — so if you cache your config, publish the current file or add the keys you need by hand.

- **A lost dispute over a sale this package routed now produces a chargeback, whichever endpoint it arrives on.** Under `separate_transfer` the platform takes the whole payment, so the charge lives on the platform account and its dispute is delivered to the platform webhook — where the mapper turned it into an add-on reversal, because nothing in the payload says the sale was routed. That reversal ends access and writes a local credit note, and it reaches none of the four things a chargeback owes a routed sale: the provider's dispute fee booked as the platform's own inbound supply, the chain corrected on the leg the ground code names, and the merchant's share taken back. The platform had returned the money to the network *and* already paid the merchant; both stayed that way, and the journal said nothing about either. The mapper now asks the routed-charge row — the only thing that knows — and emits the neutral chargeback with the merchant resolved from that row, the dispute's own reference, the ground code as the provider stated it, and the dispute fee. **A sale that was never routed is unchanged**, which is every sale on a single-seller installation. ⚠️ **This was unreachable before hosted sales could be routed at all**: until `separate_transfer` was permitted on the hosted lanes, no such sale existed to be disputed. The way in was built and the way back was not.

- **The dispute fee is read in one place instead of two.** Both webhook mappers need it and both parse the same object; the parse is one this package has already got wrong once, when it read a singular key that a Dispute does not declare and so reported no fee on every real webhook. `StripeDisputeFee` now holds it, with the reasoning that goes with it — summed rather than first-of-list, magnitude only at the end, absent rather than zero.

- **The chargeback clawback and the correcting documents fire once per DISPUTE, not once per delivery.** Both effects were deduped on the provider's event id, which collapses a redelivery of one event and nothing else. Two *different* events about the same lost dispute therefore each ran: the clawback opened a second reversal attempt, and because the provider's idempotency key is derived from that row, the second reversal carried a key the provider had no reason to refuse — the merchant's share came back twice. The chain corrector issued a second pair of correcting documents for the same sale, each adding up on its own. `RecordProviderFee`, on the same event, had already reached the other conclusion and keys its row on the dispute; all three now name the claim the same way, so they agree by construction rather than by being kept in step. **Nothing wrong has shipped:** one driver produces this event per provider today, and a genuine redelivery keeps its event id, so the second run was not reachable — it becomes reachable as soon as a dispute on a charge the platform itself took is heard by a second endpoint. ⚠️ **The key is the dispute and falls back to the charge**, because a charge carries more than one dispute only where the provider names them; an event dispatched by hand keeps the coarser claim it always had.

  **Upgrade:** the dedup key for these two effects changes, so a dispute already handled under its event id is unknown under its new key. A delivery *replayed by hand* for such a dispute would run the effect a second time. The provider does not redeliver what it already answered with a 2xx, and `billing:webhooks:replay` only replays runs that failed — but if you resend a dispute webhook from a provider dashboard for a sale whose clawback already settled, check `billing_refund_attempts` afterwards.

- **A tip now clears a server-side floor, the way a pay-what-you-want price always has.** The floor beside it carried a comment explaining why it exists — a price the buyer picks is the one place the package's stance against injection would otherwise lapse — and that argument is about the amount being the buyer's, which a tip is by construction; the contract says so in as many words. It was enforced on one of the two entries, so a tip of one cent was priced and charged. A tip under the provider's own fee pays the merchant less than nothing and still produces a document over a taxable inflow. ⚠️ **The floor had to reach three lanes, not one:** the preview prices through `FanChosenPricing`, the token charge derives its own split and asks that class only for the tip rate, and the hosted checkout opens a session from an amount with no rate to price against at all. A floor in the pricing method alone would have guarded the preview and let both money paths past — a rule with one caller and two ways around it. All three now ask the class that owns it, and each has its own arm.

  **New setting:** `billing.marketplace.tips.minimum_minor` (`BILLING_MARKETPLACE_TIPS_MINIMUM_MINOR`), in minor units, refused with the existing `FanPriceTooLow` so a consumer keeps the exception it already catches. **Null, the default, inherits `pwyw.minimum_minor`** — an operator who has said a buyer-chosen amount below X is not worth a transaction has said something the provider's fee makes true of both entries. Set it to let a voluntary payment be lower than a purchase, or to `0` to carry no floor on tips where a sale carries one; null and zero are deliberately different answers.

  **Upgrade:** on the shipped defaults nothing changes, because `pwyw.minimum_minor` is `0`. **If you have configured a pay-what-you-want floor, tips below it now throw** where they were charged before. Set `tips.minimum_minor` to `0` to keep the old behavior for tips, or to your own threshold.
...
- **A tip can be taken through a hosted checkout, so offering one no longer means building a card form.** `FanPayment::tip()` was complete and unreachable for anybody without a PCI surface: it ends in `RoutedPayment::charge(..., string $token, ...)`, and a token is produced by a card field in your own page. The whole value of the `Checkout` and `OneTimeCharge` seams is that card data never reaches a consumer — and a tip had no way through either of them, so offering one meant taking on a scope decision this package otherwise absorbs. `OneTimeCharge::tip()` opens a hosted session for a buyer-chosen amount, routes it to the merchant and writes the sale down; the confirmation webhook settles it exactly as it settles a routed add-on.

  **It is `purchase()` with the catalog removed, and the server takes over what the catalog was guarding.** `purchase()` takes a KEY and never an amount, deliberately, so a caller cannot inject a price. A tip has no catalog entry and cannot have one, so the two rules are opposite and they are two methods rather than one with a flag. What replaces the catalog is a set of refusals that all happen before the provider is asked for anything: tipping switched off, an amount that is not positive, and a sale with no merchant to route to. That last one is not symmetric with a purchase and the asymmetry is the point — an unrouted purchase is an ordinary single-seller sale and the platform keeping the money is right, while an unrouted tip is money the buyer meant for a person, kept by the platform, with neither side ever finding out.

  **The tip's own commission is used, not the platform's ordinary one.** `billing.marketplace.tips.commission_bps` existed and only the token lane honored it; a hosted tip would have been charged at the ordinary rate, which is the same figure in both places until somebody sets the tip rate — so nothing would have looked wrong. The rule stays where it lives, in `FanChosenPricing`, and the lane asks the sale context for it rather than reading the switch a second time. No buyer fee is added to a tip either: a buyer fee is charged on top of a quoted price, and a tip has no quoted price — the total is what the buyer typed.

  **What the confirmation cannot re-derive travels in the session's metadata.** A tip has no tax treatment of its own — it is placed by the supply it accompanies — so the archetype it was paid on is known when the session opens and nowhere afterwards. It rides along with the merchant and the withdrawal-declaration key, and `checkout.session.completed` maps a tip session to a new `FanTipPaid` event rather than reading it as an add-on purchase, which would have sent it to the grant path to look up a catalog key that does not exist. `billing.marketplace.tips.line_name` names the line the buyer sees, because a hardcoded English word would sit on a checkout in any other language.

- **A margin document names the goods class it was actually sold under.** The margin scheme has three, and the directive gives each its own mandatory statement on the document (Art. 226(14); § 14a Abs. 6 UStG in Germany): second-hand goods, works of art, and collectors' items and antiques. The package shipped **one** — every margin document said "second-hand goods", so an antique went out carrying the wrong mandatory statement, in the field an auditor reads first. The e-invoice half had the same single answer: `VATEX-EU-F` for all three, where a work of art owes `VATEX-EU-I` and a collectors' item `VATEX-EU-J`.

  **The goods class is a case on `TaxationBasis`, not a column beside it**, and that choice is what keeps every document already issued correct. `margin` stays `margin`: every row ever written under it *was* a second-hand goods sale, because that is the only wording the package shipped. A new column would have had to read absent as second-hand — a guess dressed as a default — while this reads the stored value as exactly what it always meant. No migration, and `taxesMarginOnly()` already existed for precisely this: its own docblock predicted "the day a jurisdiction adds a second margin-style basis", and every call site in the package asks through it rather than comparing against the case.

  **The unsuffixed translation key is kept**, deliberately. `margin_scheme_note` has shipped for releases, so a host that ran `vendor:publish` carries it under that name; renaming it would leave their own translation on disk and silently unused. The two new classes get their own keys, in all seven languages, and the profile spells all three literally so the reachability guard can see them.

  **Upgrade:** nothing to do unless you sell art or antiques. If you do, freeze the sale with `TaxationBasis::MarginWorksOfArt` or `TaxationBasis::MarginCollectorsItems` instead of `Margin`, and the document and the e-invoice follow. A host that overrode `billing::invoice.margin_scheme_note` keeps its wording for second-hand goods and should add the two new keys.

- **A merchant-issued discount code can finally discount something on a hosted checkout.** The merchant scope for coupons was complete and reachable: a creator's row is found before the catalog, and the resolver reads it. The checkout still charged full price, because the driver needs the PROVIDER's coupon and nothing in this package ever wrote `provider_coupon_id` — searched across the whole tree, the column had no writer outside the migration and `$fillable`. With it empty the driver falls back to an unscoped configuration map that a merchant code is not in, so the buyer typed the code, saw it accepted, and paid the full amount with nothing thrown and nothing logged. The two ways out were both wrong: an operator minting a coupon by hand for every creator code, or an application writing provider API calls the package exists to absorb.

  **`MerchantCouponProvisioner` is the missing seam, cut the same way `MerchantPriceProvisioner` already cuts the price side.** The row stays the host's — a discount is a commercial decision and its catalog is theirs — and the provider object becomes the package's job, because it is a provider operation needing the driver's client. Idempotency is the coupon's id rather than a lookup, since a provider coupon has no lookup key: the id is computed from everything a coupon cannot later change — issuer, code, type, value, currency, duration and cycles — so asking twice returns one coupon, and a row whose value changed asks for a different one. That is the invalidation a repriced discount needs, without a second mechanism to keep in step.

  **The account is not a choice, and the refusal is the feature.** A coupon has to exist on the account its session runs on, which is the account the price was minted on. Under a posture that mints on the merchant's connected account this refuses rather than minting on the platform anyway — a coupon on the wrong account is not an error at the provider, it is a session that silently applies no discount, which is the defect being fixed. The redemption cap and the expiry are not sent either: the row already enforces both, and a second set of numbers at the provider would drift the moment somebody edits the row.

### Fixed

- **The local quality gate could measure a different toolchain than the one CI runs, and nothing said so.** This package ships no lockfile — correct for a library, since consumers resolve their own — so `vendor/` on a machine is whatever its last install produced, while CI resolves fresh on every run, and nothing re-read the two against each other afterwards. A static analyzer older than the exactly pinned one does not fail: it answers *differently*, and the local run reports that different answer as green. Measured on a tree two analyzer releases behind the pin — it could not derive a relationship the pinned version derives, so a condition was written to state the relationship, and the pinned analyzer then rejected that same condition as redundant. The change written to make the gate and a developer machine agree was the one that made them disagree, and every step of it was green locally. An arm now reads every exactly pinned `require-dev` entry out of `composer.json` and holds the installed tree to it, naming the package, both versions and the command that closes the gap. On CI, where the two match by construction, it costs nothing.

- **Static analysis is pinned to the PHP version this package promises to run on — and the pin is credited with less than it was.** With no pinned version, the analysis is about whatever version the process happens to be running on, which is decided by whoever started it; a package is right about the lowest version it supports or it is not right at all, so the pin tracks the `require` floor and two arms hold it there, one reading the floor out of `composer.json` and one holding the CI configuration to inheriting rather than setting its own. What the first version of this entry got wrong was the reason: it reported three type holes a developer machine saw and the gate did not, and blamed the unpinned target. The split was the analyzer version, measured afterwards by removing the pin entirely on the pinned analyzer and finding the analysis clean. Two of the three shapes stay as written, because a relationship a reader has to reconstruct is worth stating whether or not a tool needs it; the third was an `instanceof` the pinned analyzer proves always true, and it is gone. Nothing about the package's behavior changes.

- **An installation that sells account credit and issues no vouchers was never told it had crossed the prepaid-value threshold.** The rolling volume figure counts two instruments — vouchers issued and paid money-credit top-ups, which are the same thing in two tables: prepaid value, held by the issuer, redeemed later. The daily sweep around that figure took its currencies from the voucher table alone, so an installation with no vouchers had an **empty** list of currencies to ask about: the number was right for `EUR` and nothing ever asked it. The sweep ran green every morning over nothing, which is exactly the state it was written against — learning at an audit that the line was passed eleven months ago and nobody was counting. Both instruments name their currencies now, and a currency carried by both is visited once, because the marker that stops a daily repeat is written after the first announcement and a second visit would deliver the duplicate before reading it. Neither list is configured: a hand-kept currency list fails silently, and a currency somebody sells in but forgot to add looks exactly like a currency under the threshold.

  **The compliance page now also names the limb of the exemption the package does not wire.** Everything it said was about what a balance or a voucher can *do* — cash out, transfer, earn interest. The exemption also rests on the instrument being redeemed where it was issued, and an instrument redeemable at a third party is a different question, one whose answer depends on arrangements only an operator makes. It is written down precisely because nothing in the package will go red about it.

- **A routed sale paid on the provider's own checkout page now leaves the buyer a document.** There are two ways such a sale happens here and only one issued anything: the token lane ends in the buyer's receipt, and its own comment says why — the money moved because a supply happened, and a supply that produces no document is the defect that method exists to end. The hosted lane took the payment, wrote the ledger row correctly, and issued nothing. Nothing went red, and the absence is the kind that surfaces at an audit or never.

  **It is issued from the completed checkout session, which turns out to carry more than the payment confirmation does:** the buyer through the customer reference, the product's archetype through the add-on key, and the place from the address the buyer actually entered rather than one anybody guessed beforehand. It runs only for a session the provider reports as paid, so it is still a document about a payment that happened — the property the synchronous lane waits for, reached the other way round. A redelivery finds the document that exists instead of drawing a second number out of a gapless series.

  **Five things stop it, and each is a fact it cannot invent.** A purchase with no routed sale behind it is an ordinary single-seller one. A posture other than the platform being the deemed supplier keeps the same asymmetry the synchronous path keeps — where the merchant is the seller of record, a document in the platform's name would name the wrong seller. A sale the provider computed tax on stops it too, and that one is a limit rather than a rule: the provider computes the rate from the buyer's address, this package then holds amounts and no rate, and a receipt stating a rate nobody supplied is the same defect one field over. An add-on nobody classified stops it, because a treatment is not one unknown but five guesses. And a buyer this application does not own is met with the same silence every other effect keeps.

- **A hosted tip told every listener the buyer was foreign, because the fact was read from a key nothing ever wrote.** `FanTipPaid` carries whether the small-value rules of the seller's own country apply, and the webhook mapper read it from `tip_buyer_domestic` on the checkout session. The session opener wrote three keys and that was not one of them — so on every real tip the flag arrived off, under a reader comment calling that "the conservative direction". Conservative is the right instinct about an absent value and the wrong word for one nothing could produce: a listener reading `false` was told something about the buyer that nobody had established. Nothing went red, because the only arm on the reader built the metadata itself and so proved the reader could read a value only the test produced. The opener states it now, from the buyer's country as the caller gave it against `billing.company.country`, and states nothing where the caller named no country — so `FanTipPaid::$buyerIsDomestic` is nullable and `null` means "not established" while `false` now means two countries were compared and differed. The arm that holds this takes the bag the driver really sends and puts THAT through the mapper, which is the only shape that could have failed the way the lane failed.

  **Upgrade:** if you listen for `FanTipPaid`, the property is `?bool` rather than `bool`. A strict comparison against `false` keeps working and now means what it says; `if (! $event->buyerIsDomestic)` will also take the unknown case, which may or may not be what you want.
...
- **The margin-scheme parts are documented for a reseller that issues its own document, and they refuse what they used to answer.** `MarginSchemeAvailability`, `MarginTaxation` and `MarginRefundCorrector` were built and never called. The package issues no goods document in anybody's name, so their caller is a consumer that issues a margin-taxed document itself, and the taxes page now says how. `MarginRefundCorrector` refuses a document that was not taxed on the margin, and one with no frozen `margin_minor`, where it used to correct on a margin of zero and give no tax back at all. A margin-taxed document that states a tax amount is refused when it is created, not only when it is rendered.

- **A monthly average rate is the one an authority announced, imported from a file — never one this package computed from the daily series.** The reader used to take the arithmetic mean of the central bank's published days for the month and hand it back under the monthly rule. That is the aggregation the ministry's own table is made of, and it is still not that table: it parts company with the announced figure the moment a day is missing locally, a publisher revises an observation, or the rounding differs by a digit — silently, on a document checked years later against the official table. The new `billing:exchange-rates:import-file` reads announced averages from a delimited file (`from,to,month,rate`) under a publisher you name with `--source`, because the ministry's portal refuses automated retrieval and working around a bot defense does not belong in a published package. **A month with no announced average now refuses the conversion instead of computing one.** If you issue documents under a monthly-average rule, import the figures before your next document.

- **A reporting renderer is handed only the sellers who must be reported, unless it is a record of the due diligence.** `render()` used to receive every seller the period examined, and the only safeguard against transmitting a seller the duty does not cover was a paragraph asking you to filter on `reportable()`. A renderer bound to `RendersReportingRecord` now receives the reportable sellers and their records and nothing else, and `seller_count` on its archived record counts those. A renderer that keeps the record of the due diligence, which needs every examined seller, implements the new `RendersDueDiligenceRecord` and is handed all of them, as the shipped delimited record is. **If a renderer of yours relies on receiving every seller, implement `RendersDueDiligenceRecord`**, and only for an artifact nobody transmits.

### Fixed

- **Credit the provider spends on an invoice now comes off this package's ledger.** A driver with a customer balance mirrors credit onto the provider so it reduces the next invoice by itself, and `CreditSync` calls this package's ledger the source of truth — but the mirror only ever pointed one way. Measured: the only production caller of `CreditLedger::debit()` was a refund, the only caller of `spendUpTo()` is the local billing engine (which never runs on a lane where the provider raises the invoice), and nothing in the package read the balance fields off an invoice at all. So from the first provider invoice that consumed credit onward, the local balance was too high — permanently, growing with every such invoice, and nothing went red. It is the balance the account hub shows the customer and the one a locally collected charge spends from, so a customer could be handed the same hundred euros twice. A finalized invoice now carries what it consumed (`starting_balance` and `ending_balance` on Stripe, floored at zero so an invoice that ADDS to the balance is not read as a debit), and a new effect takes it off the ledger under its own reason, `provider_invoice_offset` — kept apart from the local engine's `charge_offset` because one is credit this package decided to spend and the other is credit it learned had been spent. Idempotent on the invoice record: a redelivery, and the finalized-then-paid pair, debit once. **If you use a driver with a provider balance, existing balances are not corrected retroactively — the figure is right from the next such invoice on.** A driver without one is untouched, because nothing was ever mirrored there.

- **The prepaid-volume counter sees paid credit top-ups, not only vouchers.** `VoucherVolumeMonitor` summed the face value of rows in `vouchers`, and a paid credit top-up writes no such row — it credits the money balance through the add-on path. Both are the same instrument in two tables: prepaid value, held by the issuer, redeemed later against something the issuer supplies, and a supervisory threshold asks for the total value of the payment transactions over a rolling window rather than for which table this package keeps them in. So an installation that sells credit and issues vouchers only incidentally could pass the threshold while the counter stayed calm — the exact state the counter was written against. The figure now adds both sources; a refunded top-up comes off, cumulatively, so a partial refund and a full one are both covered; and currencies stay apart rather than being converted at a rate nobody published. An add-on that grants usage units is a purchase of something and is deliberately not counted — the same reading the hosted checkout uses to decide whether to tax it at the till, held in one place so the two answers about one sale cannot disagree. **If you sell credit, the figure you see will be larger than before, and it is the one the threshold is measured against.**

- **A paid credit top-up is no longer taxed when it is bought and again when it is spent.** Under a provider tax mode the hosted one-time checkout asked the provider to tax every add-on, including one that grants no usage units — which credits the owner's money balance at face value. That balance is a voucher: nothing about the supply it will eventually buy is decided yet, and the invoice it later pays carries its own tax, so the same money was taxed twice with no way to correct it afterwards. A money-credit top-up is now untaxed at the till unless the installation states that it issues single-purpose vouchers (`billing.marketplace.vouchers.instrument_type`), where the supply is determined at issue and the tax belongs there. An add-on that grants units is unchanged.

- **A creator who supplies the buyer is counted toward the small-business limit on the whole price less their own tax, not on the payout.** The earnings counter and `SmallBusinessThresholdMonitor` counted every routed sale at its payout, which is right only under the commission chain (`platform_deemed_supplier`), where the creator supplies the platform. As a seller of record or through a disclosed intermediary the creator supplies the buyer, and the platform's commission is a service bought from the platform that stays in their turnover. Those creators were under-counted by the whole commission, in the direction that takes them out of the small-business regime too late. Each sale now records its seller posture (`seller_posture` on `billing_merchant_charges`), refunds come off on the same basis, and a sale recorded before the column existed is counted under the configured posture. **On an installation that uses `seller_of_record` or `platform_intermediary`, creators cross their limit earlier than before, and that is the correction.** Run the migrations.

- **The admin console's activity table can be scrolled with a keyboard.** On a narrow screen the table is wider than the box around it and scrolls sideways, but the box could not take focus and held nothing that could, so a keyboard user could not reach the columns past the edge. The box is now focusable, named after the section's heading, and shows a focus ring. A view you published keeps the old markup until you publish it again.

- **The admin console's DATEV form lays out the same in every engine on a phone.** Its two date fields took the width their engine gave them, 155 pixels in Blink and 113 in WebKit, so on a 320-pixel screen the form broke into three rows in one engine and two in the other. It now stacks at full width on a phone and forms one row from the `sm` breakpoint, like the two forms around it, and on a phone its date fields use the larger text that keeps iOS from zooming on focus. A view you published keeps the old markup until you publish it again.

- **The admin console's form rows are one height.** From the `sm` breakpoint each form is one row, and each submit button sat 2 pixels lower than the fields beside it, because only the fields had a border. Every control in those rows is now 40 pixels tall by declaration, which also covers WebKit's Linux build, where a date field comes out 40 pixels tall on its own. A view you published keeps the old markup until you publish it again.

- **A merchant's share that fails to move after the buyer paid is kept, announced and retryable, instead of lost.** On a separate transfer the share moves in a second call after the payment. When that call failed, `RoutedPayment::charge()` threw at a caller holding a charged buyer, and the sale's row stayed `pending` for good, indistinguishable from a payment still clearing. The failure is now reported to your exception handler, written to the row (`transfer_failed_at`, `transfer_failure`) and dispatched as `MerchantShareNotMoved`, and `charge()` returns the successful payment. `billing:doctor` reports every such sale, and the new `billing:marketplace:retry-transfers` moves the shares under the idempotency key the sale used. Run the migrations.
- **A separate-transfer sale on an installation that cannot move shares is refused before the buyer is charged.** The refusal used to come after the provider call, with the payment already taken.

- **A margin-taxed document renders as category `E` with `VATEX-EU-F` in XRechnung and ZUGFeRD, not as zero-rated `Z`.** Both e-invoice writers ignored `taxation_basis`, so a margin-taxed resale went out saying it was taxed at a rate of zero, while its PDF printed no tax, no rate and the prescribed wording. The exemption reason now carries the wording the jurisdiction profile prescribes. A margin-taxed document that also names an exemption, or whose lines carry a tax rate, is refused rather than rendered.
...
- **`BillingMetricsReporter::compute()` takes a merchant.** MRR, active subscriptions, trials, dunning and churn existed platform-wide only, while the money side of a marketplace was already readable per merchant — the charge ledger, the annual earnings counter, the settlement inflow. That was not a boundary anybody drew; it is where the marketplace grew past an older reporter. It belongs in the package rather than in your application because MRR is a definition and not a query — which statuses count, how a yearly plan breaks to the month, how a trial counts — and counting per seller yourself produces a second monthly revenue that diverges from the platform's the first time either side changes a status rule. A null merchant means EVERY merchant, which is the existing behavior unchanged; `MerchantScope::platform()` asks for the platform's own rows.

- **A Mollie next-generation webhook this package has decided not to act on is now silent.** Mollie delivers on two channels: payment status still comes on the legacy one as a bare id to fetch, and the next-generation channel carries thirty-four event types of which **not one** is a `payment.*`. Every one of them therefore produced nothing here — and said so by warning that the delivery "named a resource that is not a payment", on every delivery, for types somebody had deliberately declined. A warning that fires on the ordinary case is one nobody reads by the time it carries something. There are now three states instead of two: acted on, **declined** (silent, because a person looked at it), and unclassified (still warns, because Mollie adding an event type and this package never noticing is worth a line). The ten families and the reason for each are in the troubleshooting guide; `dispute.*` and `payout.*` are declined **for now** and say what would reopen them. The universe of types is read from the SDK's own list, so one added tomorrow shows up as unclassified rather than matching a hand-copied literal nobody updated — and a test fails on it before any log does.

- **Mollie's traffic now goes through Laravel's HTTP client instead of around it.** Without an adapter the SDK picks its own transport — Guzzle if the class is there, raw cURL otherwise — and both are invisible to your application: `Http::preventStrayRequests()` did not catch a Mollie call, which is the one assurance that method exists to give; global `Http` middleware saw every outbound request except this one; and timeouts came from the SDK's defaults rather than your configuration. Nothing was broken by it, which is why it was easy to miss — a package quietly opting out of instrumentation you cannot see it opting out of. No new dependency: `illuminate/http` was already required and this package already used the facade in four other places. `StrayRequestException` is deliberately not caught, and an HTTP error status is still handed to the SDK so its own error bodies keep producing precise exceptions. Mollie's integration telemetry now sees `Laravel/<version>` as the transport rather than Guzzle's, read from Composer's runtime — this package calls no Foundation helper, and asks for both `illuminate/http` and `laravel/framework` because the framework replaces the split and Composer then reports the split as absent.

- **`ArrearsRoster` — the reminder half of the dunning ladder is now adoptable too.** `ArrearsClock` answers about ONE relationship, which is what a gate needs; a sweep has nobody in particular and has to find them, so it queried `billing_subscriptions` directly. An application holding its own view of a subscription could therefore adopt the lockout and not the reminder — and the reminder is the half that helps the person: without it the cure window runs out, access is gone, and nobody heard anything. Bind an `ArrearsRoster` and `billing:dunning:remind` works entirely over your storage. Reading and marking are one seam on purpose: the marker for "reminded today" lives wherever the rows live, and an implementation that had one without the other would send fourteen messages in a seven-day window. `LocalArrearsRoster` is the shipped default and carries the query the sweep used to run inline, so an unmodified installation is unchanged.

- **`MerchantScope::fromUid()` reads a stored `merchant_uid` back into a scope.** The rendering was always lossless, but nothing read it back — so anything holding only the sentinel could scope a query and could not say whose relationship it had, which is exactly the shape a sweep needs. Splits on the first `#` (a merchant type contains none, a key may), and anything malformed reads as the platform rather than inventing a merchant.

- **`Subscription::owner()`.** Five other models in this package already declared the same relation over the same two columns; this one did not, so anything needing the person behind a subscription resolved `owner_type` and `owner_id` by hand.

- **`billing.schema.host_key_type` — the package can now be installed by an application whose models are keyed by UUID or ULID.** Every column referencing one of your models was declared `bigint` by `Blueprint::morphs()`. For a UUID-keyed application that is not inefficient, it is fatal: `SQLSTATE[22P02]: invalid input syntax for type bigint`. Every table keyed to a customer or a merchant was therefore unwritable, which is every table that matters, and it could not be worked around from outside because the columns come from this package's own migrations. Set the key to `uuid` or `ulid` before you migrate and all 43 host references across 36 migrations follow it — 16 required morphs, 18 optional ones, and 9 halves of pairs that are declared by hand because their index needs explicit lengths. The default is `int`, so an existing installation's schema is byte-identical to what it already has. `billing:doctor` reports a setting that disagrees with the tables — it is read when a table is CREATED, so turning it on afterwards is a data migration rather than a settings change, and without the check the disagreement first appears as a failed write that names a bigint column and no setting.

- **A coupon now has an issuer, so a merchant can run their own discount code.** `billing_coupons.code` was globally unique, which is a stronger statement than "there was no merchant column": two sellers could not both offer a `SUMMER25`, so the discount namespace belonged in practice to whoever claimed a name first. On a marketplace the seller's own code is the acquisition tool — it is the reason a seller advertises their subscription page anywhere at all — so a shared namespace did not limit the feature, it removed it. Coupons are now unique per `(merchant_uid, code)` and read through `Coupon::issuedBy()`; a coupon created without an issuer belongs to the platform, which is what every coupon meant before this column existed, so an existing catalog is unaffected. `CouponRedeemer` refuses to spend one seller's coupon on another's sale — that is a transfer between two merchants neither agreed to — and reports it as `CouponUnavailable::notIssuedBy()` rather than as "not active", which would send whoever debugs it looking at flags that are all fine.

- **`RetentionHold` — an application can now hold a record back from `billing:prune`.** The pruner deletes on age, which is the storage-limitation duty and is not optional; a legal hold points the other way for one record at a time, and only the application knows about it, because the application received the letter. Until now that left two positions and both breached something: leave the pruner running and destroy evidence somebody was ordered to preserve, or switch it off and over-retain everything else. Bind a `RetentionHold` and the pruner asks before every destroying step — the stored webhook payloads, the retained financial records of erased owners, the audit ledger and the produced export documents. It is asked in bulk, one question per table rather than one per row, so a hold table answers with a single `whereIn`. Fail-closed: an implementation that throws prunes nothing from that table and the command exits non-zero, because a hold that fell back to "nothing is held" when it broke would destroy precisely the records it was installed to protect. With nothing bound, `NoRetentionHolds` holds nothing and the pruner behaves exactly as before.

### Changed

- **`PaymentReminderDue` carries an `ArrearsEntry`, and its `$subscription` is now nullable.** The entry holds the owner, the merchant and the date the arrears began — everything a message needs. `$subscription` is still populated on the shipped roster, so a listener written before this keeps working untouched; it is null only when the application has bound its own `ArrearsRoster`, because then there is no row in this package's table to carry. Constructing the event positionally changes: the entry comes first.

- **`PaymentReminderSweep` takes an `ArrearsRoster` as a third constructor argument.** Resolved from the container, so code that resolves the sweep (or runs `billing:dunning:remind`) needs no change; only a hand-rolled `new PaymentReminderSweep(...)` does.

- **`SubjectScopedRecords::pruneExpired()` takes the hold gate as a fifth argument.** Resolved from the container by the command, so an application that runs `billing:prune` needs no change; only a direct call to that method does. It is required rather than optional on purpose — a seam a caller may omit is a seam that quietly stops being asked, and the symptom here is a destroyed record nobody can recover.

### Fixed

- **The marketplace documentation said no driver routes money, and one does.** `StripeDriver` implements `RoutesMoney` and answers `marketplaceRails()`; the overview page still described the shipped driver as single-seller and told readers a marketplace needed a driver they would have to write themselves. It had been true and stayed on the page after the driver gained the interface — the expensive direction for a stale sentence, because it does not fail, it sends somebody to build something that already exists. The page now states the requirement plainly: the marketplace branch needs the Stripe driver, and `MollieDriver` deliberately does not route (its own docblock has always said why).

## [0.20.1] - 2026-09-06

### Fixed

- **Two documentation blocks now read US English.** `RefundKind` and `ChargeNarrative` each carried one word in its British form; they say `mislabeled` and `recognizable` now. Docblocks reach every reader through their editor and through the distributed source, and this package writes US English throughout — three other files already spelled the second one the American way, so these two were the odd ones out rather than a different convention.
